...
temp_post_fv = simlog.Frost_valve.Frost_valve_coil.A2.T.series.values('degC');
OAT = simlog.Fresh_air_supply.A.T.series.values('degC');
% Given parameters
...
T_room_initial = 18.5; % Initial room temperature (°C)
...
for i = 2:length(time)
    ...
    Q_in = (h * A * (operating_temperature(i) - T_room(i-1)))+25;
...
grid on;
t = simlog.Frost_valve.Frost_valve_coil.Q.series.time;
power_frost_cl = simlog.Frost_valve.Frost_valve_coil.Q.series.values('kW');
power_cooling = simlog.Cooling_coil.Cooling_coil.Q.series.values('kW');
power_heating = simlog.Heating_coil.Pre_heating_coil.Q.series.values('kW');
total_power = power_frost_cl-power_cooling+power_heating;
filename = 'Matlab outputs.xlsx';
writematrix(t,filename,'Sheet',1);
writematrix(T_room,filename,'Sheet',2);
writematrix(total_power,filename,'Sheet',3);
writematrix(temp_post_fv,filename,'Sheet',4);
writematrix(OAT,filename,'Sheet',5);
